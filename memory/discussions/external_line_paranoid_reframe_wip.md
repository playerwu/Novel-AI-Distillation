# 外线 paranoid reframe 讨论稿（已归档 · 被 v5 综合超越）

> **状态**：**已归档**。本稿是 v4 → v5 过渡期的外线 paranoid framework 探索 WIP，结论在 [digital_persona_design_v5.md](digital_persona_design_v5.md) 第七章 7.4 节落地——paranoid 不作为 framework（动机层贡献为零），但可以作为 AI 中段挣扎的可选形态保留（dramatic 厚度）。本稿仅作为设计沿革保留，理解项目请直接读 v5。
>
> **被超越的关键原因**：本稿当时三个动机问题（AI 关机动机 / 妻子按按钮动机 / 项目临床 thesis）都没解决。v5 通过反转关机发起者（AI 不请求关机 → 妻子在仪式性瞬间主动按下，AI 抗拒+央求）一次性解开三个死结。paranoid 在新 framework 下不需要 load-bear 关机 reasoning，可作为 AI 中段挣扎的可选形态自由叠加。
>
> ---
>
> **以下为本稿原始内容（已被 v5 超越，仅作设计沿革保留）**：
>
> **原始状态**：讨论中，**未达成最终设计**。用户明确表示"还不完备"，下次继续。本稿仅记录当前已推到的位置，不进入 v4 主稿，不修订人物卡。
>
